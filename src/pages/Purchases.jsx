...
const Purchases = ({ user }) => {
  ...
  const [items, setItems] = useState([
    { product: '', batchNumber: '', expiryDate: '', quantity: 1, rate: 0, gstPercentage: 18, total: 0 }
  ]);
  ...
  const handleSubmit = async (e) => {
    ...
    const invalidItem = items.some(item => !item.product || !item.batchNumber || !item.expiryDate || item.quantity <= 0 || item.rate <= 0);
    if (invalidItem) {
      setError('Please fill all item lines completely. Batch Number, Expiry, Qty and Cost must be valid.');
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setItems([{ product: '', batchNumber: '', expiryDate: '', quantity: 1, rate: 0, gstPercentage: 18, total: 0 }]);
      } else {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ fontSize: '1.75rem', marginBottom: '4px' }}>Purchases & Stock Inward</h1>
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
            Record product import receipts from manufacturers, specify batches/expiry, and auto-increase inventory stock.
          </p>
        </div>
            ...
            <tbody>
              {purchases.map(p => (
                <tr key={p._id}>
                  ...
                  <td style={{ fontWeight: '600' }}>{p.company?.name || 'Unknown'}</td>
                  <td style={{ fontFamily: 'monospace', fontWeight: 'bold' }}>{p.invoiceNumber}</td>
                  <td>₹{p.subtotal.toLocaleString('en-IN')}</td>
                  <td style={{ color: 'var(--text-secondary)' }}>₹{p.gstTotal.toLocaleString('en-IN')}</td>
                  <td style={{ fontWeight: '600', color: 'var(--accent)' }}>₹{p.grandTotal.toLocaleString('en-IN')}</td>
                  <td>
                    <button 
          ...
          <div className="modal-content" style={{ maxWidth: '850px', width: '95%' }}>
            ...
            <form onSubmit={handleSubmit}>
              <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                ...
                <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: '16px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                    <h4 style={{ fontSize: '0.9rem', color: 'var(--text-secondary)' }}>Invoice Line Items</h4>
                    <button type="button" onClick={handleAddItemRow} className="btn-secondary" style={{ padding: '6px 12px', fontSize: '0.8rem' }}>
                      + Add Item Row
                    </button>
                  </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', maxHeight: '250px', overflowY: 'auto', paddingRight: '4px' }}>
                    {items.map((item, idx) => (
                      <div 
                        ...
                        style={{ 
                          display: 'grid', 
                          gridTemplateColumns: '2fr 1.2fr 1.2fr 1fr 1fr 1.2fr 40px', 
                          gap: '10px', 
                          alignItems: 'end',
                      ...
                      >
                        <div className="form-group" style={{ marginBottom: 0 }}>
                          <label className="form-label" style={{ fontSize: '0.75rem' }}>Select Dawai SKU</label>
                          <select 
                            className="filter-select"
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', borderTop: '1px solid var(--border-color)', paddingTop: '16px' }}>
                  <div style={{ width: '250px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '1.1rem', fontWeight: 'bold', borderTop: '1px dashed var(--border-color)', paddingTop: '8px' }}>
              ...
              <div className="modal-footer">
                ...
                <button type="submit" className="btn-primary">Log Procurement (Stock-In)</button>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className="modal-content" style={{ maxWidth: '650px' }}>
            ...
            <div className="modal-body">
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '20px', fontSize: '0.9rem' }}>
                <div>
                  <span style={{ color: 'var(--text-secondary)', display: 'block' }}>Manufacturer:</span>
                  <strong>{viewingPurchase.company?.name}</strong>
                  <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>GST: {viewingPurchase.company?.gstNumber}</span>
                </div>
                <div>
                  <span style={{ color: 'var(--text-secondary)', display: 'block' }}>Invoice Number:</span>
                  <strong>{viewingPurchase.invoiceNumber}</strong>
                  <span style={{ display: 'block', fontSize: '0.8rem' }}>Date: {new Date(viewingPurchase.date).toLocaleDateString()}</span>
                </div>
              </div>

              <h4 style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', textTransform: 'uppercase', marginBottom: '10px' }}>Purchased Items</h4>
                ...
                <table className="responsive-table" style={{ fontSize: '0.85rem' }}>
                  ...
                  <tbody>
                    {viewingPurchase.items?.map((item, idx) => (
                      <tr key={idx}>
                        <td style={{ padding: '10px 16px', fontWeight: '600' }}>{item.product?.name || 'Deleted Product'}</td>
                        <td style={{ padding: '10px 16px', fontFamily: 'monospace' }}>{item.batchNumber}</td>
                        <td style={{ padding: '10px 16px' }}>{item.quantity}</td>
                        <td style={{ padding: '10px 16px' }}>₹{item.rate}</td>
                        <td style={{ padding: '10px 16px', fontWeight: 'bold' }}>₹{item.total}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '20px' }}>
                <div style={{ width: '220px', display: 'flex', flexDirection: 'column', gap: '6px', fontSize: '0.85rem' }}>
                  <div style={{ display: 'flex', justifyBetween: 'space-between' }}>
                    <span style={{ color: 'var(--text-secondary)', flexGrow: 1 }}>Subtotal:</span>
                    <span>₹{viewingPurchase.subtotal}</span>
                  </div>
                  <div style={{ display: 'flex', justifyBetween: 'space-between' }}>
                    <span style={{ color: 'var(--text-secondary)', flexGrow: 1 }}>GST:</span>
                    <span>₹{viewingPurchase.gstTotal}</span>
                  </div>
                  <div style={{ display: 'flex', justifyBetween: 'space-between', fontWeight: 'bold', fontSize: '1rem', borderTop: '1px dashed var(--border-color)', paddingTop: '6px', marginTop: '4px' }}>
                    <span style={{ flexGrow: 1 }}>Total Bill:</span>
                    <span style={{ color: 'var(--accent)' }}>₹{viewingPurchase.grandTotal}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="modal-footer">
              <button className="btn-primary" onClick={() => setViewingPurchase(null)}>Done</button>
            </div>
          </div>
        </div>
      )}
